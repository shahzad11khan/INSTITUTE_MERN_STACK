...
exports.Tutionmodel = async (req, res) => {
  console.log(req.body);
  try {
    // Create a new staff model with file metadata
    const newStaff = new Tution({
      name: req.body.name,
...
exports.getAll = async (req, res) => {
  try {
    const newstaffs = await Tution.find();
...
exports.getId = async (req, res) => {
  try {
    const newstaff = await Tution.findById(req.params.id);
    if (!newstaff) {
...
exports.updateId = async (req, res) => {
  try {
    const updatednewstaff = await Tution.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteId = async (req, res) => {
  try {
    const deletednewstaff = await Tution.findByIdAndDelete(req.params.id);
    if (!deletednewstaff) {
      return res.status(404).json({ message: "Staff member not found" });
    }
