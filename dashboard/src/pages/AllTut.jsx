...
const AllTut = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/api/tutions/${id}`, {
        method: "DELETE",
      });
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/tutions");
  ...
  const Approvehandle = async (itemId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/tutions/${itemId}`,
        {
          method: "PUT",
          headers: {
